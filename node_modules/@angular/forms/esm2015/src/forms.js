/**
 * @fileoverview added by tsickle
 * @suppress {checkTypes,constantProperty,extraRequire,missingOverride,missingReturn,unusedPrivateMembers,uselessCode} checked by tsc
 */
/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
/**
 * @module
 * @description
 * This module is used for handling user input, by defining and building a `FormGroup` that
 * consists of `FormControl` objects, and mapping them onto the DOM. `FormControl`
 * objects can then be used to read information from the form DOM elements.
 *
 * Forms providers are not included in default providers; you must import these providers
 * explicitly.
 */
export { ɵInternalFormsSharedModule } from '@angular/forms/esm2015/src/directives';
export { AbstractControlDirective } from '@angular/forms/esm2015/src/directives/abstract_control_directive';
export { AbstractFormGroupDirective } from '@angular/forms/esm2015/src/directives/abstract_form_group_directive';
export { CheckboxControlValueAccessor } from '@angular/forms/esm2015/src/directives/checkbox_value_accessor';
export { ControlContainer } from '@angular/forms/esm2015/src/directives/control_container';
export { NG_VALUE_ACCESSOR } from '@angular/forms/esm2015/src/directives/control_value_accessor';
export { COMPOSITION_BUFFER_MODE, DefaultValueAccessor } from '@angular/forms/esm2015/src/directives/default_value_accessor';
export { NgControl } from '@angular/forms/esm2015/src/directives/ng_control';
export { NgControlStatus, NgControlStatusGroup } from '@angular/forms/esm2015/src/directives/ng_control_status';
export { NgForm } from '@angular/forms/esm2015/src/directives/ng_form';
export { NgFormSelectorWarning } from '@angular/forms/esm2015/src/directives/ng_form_selector_warning';
export { NgModel } from '@angular/forms/esm2015/src/directives/ng_model';
export { NgModelGroup } from '@angular/forms/esm2015/src/directives/ng_model_group';
export { ɵNgNoValidate } from '@angular/forms/esm2015/src/directives/ng_no_validate_directive';
export { NumberValueAccessor } from '@angular/forms/esm2015/src/directives/number_value_accessor';
export { RadioControlValueAccessor } from '@angular/forms/esm2015/src/directives/radio_control_value_accessor';
export { RangeValueAccessor } from '@angular/forms/esm2015/src/directives/range_value_accessor';
export { FormControlDirective } from '@angular/forms/esm2015/src/directives/reactive_directives/form_control_directive';
export { FormControlName } from '@angular/forms/esm2015/src/directives/reactive_directives/form_control_name';
export { FormGroupDirective } from '@angular/forms/esm2015/src/directives/reactive_directives/form_group_directive';
export { FormArrayName } from '@angular/forms/esm2015/src/directives/reactive_directives/form_group_name';
export { FormGroupName } from '@angular/forms/esm2015/src/directives/reactive_directives/form_group_name';
export { NgSelectOption, SelectControlValueAccessor } from '@angular/forms/esm2015/src/directives/select_control_value_accessor';
export { SelectMultipleControlValueAccessor } from '@angular/forms/esm2015/src/directives/select_multiple_control_value_accessor';
export { ɵNgSelectMultipleOption } from '@angular/forms/esm2015/src/directives/select_multiple_control_value_accessor';
export { CheckboxRequiredValidator, EmailValidator, MaxLengthValidator, MinLengthValidator, PatternValidator, RequiredValidator } from '@angular/forms/esm2015/src/directives/validators';
export { FormBuilder } from '@angular/forms/esm2015/src/form_builder';
export { AbstractControl, FormArray, FormControl, FormGroup } from '@angular/forms/esm2015/src/model';
export { NG_ASYNC_VALIDATORS, NG_VALIDATORS, Validators } from '@angular/forms/esm2015/src/validators';
export { VERSION } from '@angular/forms/esm2015/src/version';
export { FormsModule, ReactiveFormsModule } from '@angular/forms/esm2015/src/form_providers';
//# sourceMappingURL=data:application/json;base64,
