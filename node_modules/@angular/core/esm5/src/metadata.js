/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
export { Attribute } from '@angular/core/esm5/src/di';
export { ANALYZE_FOR_ENTRY_COMPONENTS, ContentChild, ContentChildren, Query, ViewChild, ViewChildren } from '@angular/core/esm5/src/metadata/di';
export { Component, Directive, HostBinding, HostListener, Input, Output, Pipe } from '@angular/core/esm5/src/metadata/directives';
export { NgModule } from '@angular/core/esm5/src/metadata/ng_module';
export { CUSTOM_ELEMENTS_SCHEMA, NO_ERRORS_SCHEMA } from '@angular/core/esm5/src/metadata/schema';
export { ViewEncapsulation } from '@angular/core/esm5/src/metadata/view';
//# sourceMappingURL=data:application/json;base64,
