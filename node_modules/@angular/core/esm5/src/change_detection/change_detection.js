/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
import { DefaultIterableDifferFactory } from '@angular/core/esm5/src/change_detection/differs/default_iterable_differ';
import { DefaultKeyValueDifferFactory } from '@angular/core/esm5/src/change_detection/differs/default_keyvalue_differ';
import { IterableDiffers } from '@angular/core/esm5/src/change_detection/differs/iterable_differs';
import { KeyValueDiffers } from '@angular/core/esm5/src/change_detection/differs/keyvalue_differs';
export { WrappedValue, devModeEqual } from '@angular/core/esm5/src/change_detection/change_detection_util';
export { ChangeDetectorRef } from '@angular/core/esm5/src/change_detection/change_detector_ref';
export { ChangeDetectionStrategy, ChangeDetectorStatus, isDefaultChangeDetectionStrategy } from '@angular/core/esm5/src/change_detection/constants';
export { DefaultIterableDifferFactory } from '@angular/core/esm5/src/change_detection/differs/default_iterable_differ';
export { DefaultIterableDiffer } from '@angular/core/esm5/src/change_detection/differs/default_iterable_differ';
export { DefaultKeyValueDifferFactory } from '@angular/core/esm5/src/change_detection/differs/default_keyvalue_differ';
export { IterableDiffers } from '@angular/core/esm5/src/change_detection/differs/iterable_differs';
export { KeyValueDiffers } from '@angular/core/esm5/src/change_detection/differs/keyvalue_differs';
export { SimpleChange } from '@angular/core/esm5/src/interface/simple_change';
/**
 * Structural diffing for `Object`s and `Map`s.
 */
var keyValDiff = [new DefaultKeyValueDifferFactory()];
/**
 * Structural diffing for `Iterable` types such as `Array`s.
 */
var iterableDiff = [new DefaultIterableDifferFactory()];
export var defaultIterableDiffers = new IterableDiffers(iterableDiff);
export var defaultKeyValueDiffers = new KeyValueDiffers(keyValDiff);
//# sourceMappingURL=data:application/json;base64,
