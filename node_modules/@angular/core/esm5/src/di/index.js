/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
/**
 * @module
 * @description
 * The `di` module provides dependency injection container services.
 */
export * from '@angular/core/esm5/src/di/metadata';
export { InjectFlags } from '@angular/core/esm5/src/di/interface/injector';
export { ɵɵdefineInjectable, defineInjectable, ɵɵdefineInjector } from '@angular/core/esm5/src/di/interface/defs';
export { forwardRef, resolveForwardRef } from '@angular/core/esm5/src/di/forward_ref';
export { Injectable } from '@angular/core/esm5/src/di/injectable';
export { Injector } from '@angular/core/esm5/src/di/injector';
export { ɵɵinject, inject, INJECTOR } from '@angular/core/esm5/src/di/injector_compatibility';
export { ReflectiveInjector } from '@angular/core/esm5/src/di/reflective_injector';
export { ResolvedReflectiveFactory } from '@angular/core/esm5/src/di/reflective_provider';
export { ReflectiveKey } from '@angular/core/esm5/src/di/reflective_key';
export { InjectionToken } from '@angular/core/esm5/src/di/injection_token';
//# sourceMappingURL=data:application/json;base64,
