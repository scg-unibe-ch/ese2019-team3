/**
 * @fileoverview added by tsickle
 * @suppress {checkTypes,constantProperty,extraRequire,missingOverride,missingReturn,unusedPrivateMembers,uselessCode} checked by tsc
 */
/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
export { anchorDef, elementDef } from '@angular/core/esm2015/src/view/element';
export { clearOverrides, createNgModuleFactory, overrideComponentView, overrideProvider } from '@angular/core/esm2015/src/view/entrypoint';
export { ngContentDef } from '@angular/core/esm2015/src/view/ng_content';
export { moduleDef, moduleProvideDef } from '@angular/core/esm2015/src/view/ng_module';
export { directiveDef, pipeDef, providerDef } from '@angular/core/esm2015/src/view/provider';
export { pureArrayDef, pureObjectDef, purePipeDef } from '@angular/core/esm2015/src/view/pure_expression';
export { queryDef } from '@angular/core/esm2015/src/view/query';
export { ViewRef_, createComponentFactory, getComponentViewDefinitionFactory, nodeValue } from '@angular/core/esm2015/src/view/refs';
export { initServicesIfNeeded } from '@angular/core/esm2015/src/view/services';
export { textDef } from '@angular/core/esm2015/src/view/text';
export { EMPTY_ARRAY, EMPTY_MAP, createRendererType2, elementEventFullName, inlineInterpolate, interpolate, rootRenderNodes, tokenKey, unwrapValue } from '@angular/core/esm2015/src/view/util';
export { viewDef } from '@angular/core/esm2015/src/view/view';
export { attachEmbeddedView, detachEmbeddedView, moveEmbeddedView } from '@angular/core/esm2015/src/view/view_attach';
export { shiftInitState, shouldCallLifecycleInitHook, asTextData, asElementData, asProviderData, asPureExpressionData, asQueryList, NodeData, DebugContext, Services } from '@angular/core/esm2015/src/view/types';
//# sourceMappingURL=data:application/json;base64,
