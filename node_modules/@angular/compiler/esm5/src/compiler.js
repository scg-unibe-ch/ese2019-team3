/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
//////////////////////////////////////
// THIS FILE HAS GLOBAL SIDE EFFECT //
//       (see bottom of file)       //
//////////////////////////////////////
/**
 * @module
 * @description
 * Entry point for all APIs of the compiler package.
 *
 * <div class="callout is-critical">
 *   <header>Unstable APIs</header>
 *   <p>
 *     All compiler apis are currently considered experimental and private!
 *   </p>
 *   <p>
 *     We expect the APIs in this package to keep on changing. Do not rely on them.
 *   </p>
 * </div>
 */
import * as core from '@angular/compiler/esm5/src/core';
import { publishFacade } from '@angular/compiler/esm5/src/jit_compiler_facade';
import { global } from '@angular/compiler/esm5/src/util';
export { core };
export * from '@angular/compiler/esm5/src/version';
export * from '@angular/compiler/esm5/src/template_parser/template_ast';
export { CompilerConfig, preserveWhitespacesDefault } from '@angular/compiler/esm5/src/config';
export * from '@angular/compiler/esm5/src/compile_metadata';
export * from '@angular/compiler/esm5/src/aot/compiler_factory';
export * from '@angular/compiler/esm5/src/aot/compiler';
export * from '@angular/compiler/esm5/src/aot/generated_file';
export * from '@angular/compiler/esm5/src/aot/formatted_error';
export * from '@angular/compiler/esm5/src/aot/static_reflector';
export * from '@angular/compiler/esm5/src/aot/static_symbol';
export * from '@angular/compiler/esm5/src/aot/static_symbol_resolver';
export * from '@angular/compiler/esm5/src/aot/summary_resolver';
export { isLoweredSymbol, createLoweredSymbol } from '@angular/compiler/esm5/src/aot/util';
export * from '@angular/compiler/esm5/src/ast_path';
export * from '@angular/compiler/esm5/src/summary_resolver';
export { Identifiers } from '@angular/compiler/esm5/src/identifiers';
export { JitCompiler } from '@angular/compiler/esm5/src/jit/compiler';
export * from '@angular/compiler/esm5/src/compile_reflector';
export * from '@angular/compiler/esm5/src/url_resolver';
export * from '@angular/compiler/esm5/src/resource_loader';
export { ConstantPool } from '@angular/compiler/esm5/src/constant_pool';
export { DirectiveResolver } from '@angular/compiler/esm5/src/directive_resolver';
export { PipeResolver } from '@angular/compiler/esm5/src/pipe_resolver';
export { NgModuleResolver } from '@angular/compiler/esm5/src/ng_module_resolver';
export { DEFAULT_INTERPOLATION_CONFIG, InterpolationConfig } from '@angular/compiler/esm5/src/ml_parser/interpolation_config';
export * from '@angular/compiler/esm5/src/schema/element_schema_registry';
export * from '@angular/compiler/esm5/src/i18n';
export * from '@angular/compiler/esm5/src/directive_normalizer';
export * from '@angular/compiler/esm5/src/expression_parser/ast';
export * from '@angular/compiler/esm5/src/expression_parser/lexer';
export * from '@angular/compiler/esm5/src/expression_parser/parser';
export * from '@angular/compiler/esm5/src/metadata_resolver';
export * from '@angular/compiler/esm5/src/ml_parser/ast';
export * from '@angular/compiler/esm5/src/ml_parser/html_parser';
export * from '@angular/compiler/esm5/src/ml_parser/html_tags';
export * from '@angular/compiler/esm5/src/ml_parser/interpolation_config';
export * from '@angular/compiler/esm5/src/ml_parser/tags';
export { NgModuleCompiler } from '@angular/compiler/esm5/src/ng_module_compiler';
export { ArrayType, AssertNotNull, DYNAMIC_TYPE, BinaryOperator, BinaryOperatorExpr, BuiltinMethod, BuiltinType, BuiltinTypeName, BuiltinVar, CastExpr, ClassField, ClassMethod, ClassStmt, CommaExpr, CommentStmt, ConditionalExpr, DeclareFunctionStmt, DeclareVarStmt, Expression, ExpressionStatement, ExpressionType, ExternalExpr, ExternalReference, literalMap, FunctionExpr, IfStmt, InstantiateExpr, InvokeFunctionExpr, InvokeMethodExpr, JSDocCommentStmt, LiteralArrayExpr, LiteralExpr, LiteralMapExpr, MapType, NotExpr, ReadKeyExpr, ReadPropExpr, ReadVarExpr, ReturnStatement, ThrowStmt, TryCatchStmt, Type, WrappedNodeExpr, WriteKeyExpr, WritePropExpr, WriteVarExpr, StmtModifier, Statement, STRING_TYPE, TypeofExpr, collectExternalReferences } from '@angular/compiler/esm5/src/output/output_ast';
export { EmitterVisitorContext } from '@angular/compiler/esm5/src/output/abstract_emitter';
export { JitEvaluator } from '@angular/compiler/esm5/src/output/output_jit';
export * from '@angular/compiler/esm5/src/output/ts_emitter';
export * from '@angular/compiler/esm5/src/parse_util';
export * from '@angular/compiler/esm5/src/schema/dom_element_schema_registry';
export * from '@angular/compiler/esm5/src/selector';
export * from '@angular/compiler/esm5/src/style_compiler';
export * from '@angular/compiler/esm5/src/template_parser/template_parser';
export { ViewCompiler, findStaticQueryIds, staticViewQueryIds } from '@angular/compiler/esm5/src/view_compiler/view_compiler';
export { getParseErrors, isSyntaxError, syntaxError, Version } from '@angular/compiler/esm5/src/util';
export * from '@angular/compiler/esm5/src/injectable_compiler_2';
export { BoundAttribute as TmplAstBoundAttribute, BoundEvent as TmplAstBoundEvent, BoundText as TmplAstBoundText, Content as TmplAstContent, Element as TmplAstElement, RecursiveVisitor as TmplAstRecursiveVisitor, Reference as TmplAstReference, Template as TmplAstTemplate, Text as TmplAstText, TextAttribute as TmplAstTextAttribute, Variable as TmplAstVariable, } from '@angular/compiler/esm5/src/render3/r3_ast';
export * from '@angular/compiler/esm5/src/render3/view/t2_binder';
export { Identifiers as R3Identifiers } from '@angular/compiler/esm5/src/render3/r3_identifiers';
export { R3ResolvedDependencyType } from '@angular/compiler/esm5/src/render3/r3_factory';
export { compileInjector, compileNgModule } from '@angular/compiler/esm5/src/render3/r3_module_compiler';
export { compilePipeFromMetadata } from '@angular/compiler/esm5/src/render3/r3_pipe_compiler';
export { makeBindingParser, parseTemplate } from '@angular/compiler/esm5/src/render3/view/template';
export { compileBaseDefFromMetadata, compileComponentFromMetadata, compileDirectiveFromMetadata, parseHostBindings, verifyHostBindings } from '@angular/compiler/esm5/src/render3/view/compiler';
export { publishFacade } from '@angular/compiler/esm5/src/jit_compiler_facade';
// This file only reexports content of the `src` folder. Keep it that way.
// This function call has a global side effects and publishes the compiler into global namespace for
// the late binding of the Compiler to the @angular/core for jit compilation.
publishFacade(global);
//# sourceMappingURL=data:application/json;base64,
